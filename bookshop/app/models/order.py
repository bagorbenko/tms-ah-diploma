...
class Order(db.Model):
    ...
    api_store_status = db.Column(db.String(50))  # Статус отправки в API Store
    api_store_response = db.Column(db.JSON)  # Ответ от API Store
...
class OrderItem(db.Model):
    ...
    price_at_time = db.Column(db.Numeric(precision=10, scale=2), nullable=False)  # Цена на момент заказа
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
